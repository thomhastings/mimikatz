...
const wchar_t strNotSupported[] = 
	L"\n========================================\n\n\
Les fonctions suivantes NE SONT PLUS supportées via l\'injection de la librairie sekurlsa.dll :\n\n\
...
__kextdll bool __cdecl getDescription(wstring * maDescription)
{
	...
	maDescription->append(strNotSupported);
	return mod_system::getVersion(&mod_system::GLOB_Version);
}

__kextdll bool __cdecl notsupported(mod_pipe * monPipe, vector<wstring> * mesArguments)
{
	return sendTo(monPipe, strNotSupported);
}
